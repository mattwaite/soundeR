...
- [ ] `sonify_video()` (plot + soundtrack via `av`). **Design (Matt's feature request, 2026-09-23):**
  ...
  - Later: a `plot =` argument so students can build the base chart themselves (`autoplot(x) + labs(...) + theme(...) + scale_*()`), with `sonify_video()` adding only the playhead layers. This is the fully ggplot-native route.
