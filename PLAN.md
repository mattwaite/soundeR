...
- [x] **Per-instrument default ranges (built 2026-09-24).** `range = NULL` by default: one real instrument uses its usual range (rounded orchestral ranges, trimmed to about 2.5–3 octaves; family defaults plus overrides in `R/instruments.R`, e.g. cello C2–C5, tuba D1–D4, piccolo D5–C8). Several share the overlap if it's at least 18 semitones; otherwise C3–C6, with a rate-limited message. Built-in sounds use C3–C6 and don't narrow the overlap. A range you set yourself always wins. `instruments()` gains `low`/`high`. The vignette's two-score example switched from xylophone + cello (little overlap, so it would print the message) to marimba + cello (C3–C5).
- [ ] Remaining: Posit Cloud (the GitHub Actions Linux build already works); maybe cache the rendered mp3 too.
...
- [x] **`sonify_plot()` and `sonify_video(plot =)` (built 2026-09-24).** `sonify_plot(x)` returns the finished chart (every note played) as a normal ggplot, tagged with `attr(p, "soundeR_layout")` and `"soundeR_colors"` (custom attributes survive `+` in ggplot2 4.0). Video frames = that plot + `add_frame_layers()`: hollow dots / gray bars / gray curve over the notes still to play, the highlight band inserted at the **bottom** of `p$layers`, and the playhead on top, so user additions (scales, annotations, themes) stay put. `sonify_video(plot = p)` checks the plot came from `sonify_plot()` of the same sonification (onsets match); `point_color` with `plot` warns; title/label/theme args still apply on top; title position is enforced. Caveat, documented: adding a complete theme to a *static* `sonify_plot()` with `+` resets ggplot's title position, like any ggplot. Replaces the old "piano-roll `autoplot()`" idea. 20 new/updated tests.
...
- [x] **Panning (built 2026-09-24).** `pan =` in `sonify_data()`: a fixed number (-1 to 1) or word ("left", "center", "right"; any value that mentions no data column), a numeric column (rescaled -1..1), or a categorical column (spaced -0.8..0.8). **Default: several voices spread across -0.6..0.6 automatically**; `pan = 0` turns it off. Missing values go to the center, with a message. `notes()` gains `pan`. Rendering: the synth mixes to stereo with equal-power gains only when something is panned (centered pieces stay mono); the glide interpolates pan per sample; MIDI puts a CC10 on each channel, with channels keyed by instrument *and* position (`midi_pans()` coarsens positions to 17, then 5, then 3 steps to fit 15 channels, else centers everything with a message). `sonify_histogram()` and `sonify_density()` gain `pan = TRUE`, a left-to-right sweep along the x axis. 40 tests, including left/right RMS on real renders. ~~Drums/percussion~~ dropped (see Phase 3).
...
- [x] **pkgdown site (built 2026-09-24)**: https://www.mattwaite.com/soundeR/, deployed by `.github/workflows/pkgdown.yaml` (the r-lib example plus a `fluidsynth::soundfont_download()` step, so the site's players use real instruments) to the `gh-pages` branch. `_pkgdown.yml` groups the reference (make sound / see-save-inspect / instruments and scales / data); the getting-started vignette plus a "Hearing distributions" article. The README videos are now `<video src=... controls>` tags: GitHub renders them like the bare links (checked with `gh api markdown`), and they also play on the site, where bare links didn't. `pkgdown_print.sonification()` gives reference-page examples real players (build with `lazy = FALSE` after changing it). Gotchas: pkgdown renders **every** top-level `.md`, so PLAN.md and CLAUDE.md become unlinked pages (left as is; the plan is public anyway); a local build needs `textshaping` and `RSTUDIO_PANDOC=/Applications/quarto/bin/tools/aarch64`. `.claude/launch.json` has a `site` server on port 8766 for previews.
- [ ] Teaching materials, class exercise, accessibility review, and testing on other platforms: **Matt is handling these** (2026-09-24), with students on other machines.
- [ ] CRAN submission: **on hold** (Matt, 2026-09-24) until there's clear demand in the R community. The CRAN prep list is in the 2026-09-24 session log.
...
- **2026-09-24 (later)** — Built stereo panning (pushed, 3120029) and the pkgdown site. Matt takes teaching materials, accessibility and cross-platform testing; CRAN is on hold. CRAN prep, whenever it happens: vignette size (about 3.3 + 2.2 MB of embedded audio vs. the ~5 MB limit), the package-name decision, data licensing notes (Wikipedia CC BY-SA; ESPN via hoopR), win-builder/R-hub checks, NEWS.md, cran-comments.md, version 0.1.0.
- **2026-09-24 (later)** — The site is live at **https://www.mattwaite.com/soundeR/**. Matt's GitHub Pages uses a custom domain, so mattwaite.github.io/soundeR/ 301-redirects there; `_pkgdown.yml` and DESCRIPTION now use the real address. The first Actions build took 5 minutes and passed on ubuntu-latest, which is also the first proof that fluidsynth and the soundfont download work on Linux. On the live site, all players play with real instruments (no stand-in messages) and both README videos play.
- **2026-09-24 (later)** — Matt: the package name stays soundeR. Built per-instrument default ranges and `sonify_plot()` / `sonify_video(plot =)`. 466 tests.
